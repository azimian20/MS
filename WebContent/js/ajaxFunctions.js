	...
	return function() { // --handle error messages and exceptions
		if (xmlHttpRequest.readyState == 4) {
			if (xmlHttpRequest.status == 200) {
				createMovieContainerPan();
				var jsonMovieContainer = JSON.parse(xmlHttpRequest.responseText);
				...
				for (var i = 0; i < movieResultList.length; i++) {
					...
					createMovieAttribLine(movieLine, "Movie Name",
							movieResultList[i].title);
					createMovieAttribLine(movieLine, "Popularity",
							movieResultList[i].popularity);
					createMovieAttribLine(movieLine, "Overview",
				...
				createMovieContainerPan();
				var movieLine = createMovieLine();
				createMovieAttribLine(movieLine, "Error", xmlHttpRequest.getResponseHeader("errorMessage"));
				finishMovieLine(movieLine);
			} else {
...
function createMovieContainerPan() {
